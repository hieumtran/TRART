...
class menuUI:
    def __init__(self, title, start, quit, diff, back, w_screen, h_screen):
        ...
        self.title = self.transform(self.title, 1)
        self.start = self.transform(self.start, 0.8)
        # self.quit = self.transform(self.quit, 0.8)
        self.back = self.transform(self.back, 0.8)
        
        for i in range(len(self.diff)):
            self.diff[i] = self.transform(self.diff[i], 0.5)
