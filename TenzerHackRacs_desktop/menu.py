...
class menuUI:
    def __init__(self, title, start, quit, diff, back, w_screen, h_screen):
        ...
        self.title = self.transform(self.title, 0.5)
        self.start = self.transform(self.start, 0.3)
        self.quit = self.transform(self.quit, 0.3)
        self.back = self.transform(self.back, 0.3)
        
        for i in range(len(self.diff)):
            self.diff[i] = self.transform(self.diff[i], 0.2)
    ...
    def draw(self, screen, cond, pos):
        ...
        if cond == 1:
            ...
            back_rect = self.back.get_rect(center=(self.w_screen/2, up_height+self.back.get_height()))
